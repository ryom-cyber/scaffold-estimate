...
export default function ScaffoldPlan({ layout }: Props) {
  ...
  // ──────────────────────────────────────────
  const handlePlanPDF = () => {
    const planSvgEl  = document.querySelector('.scaffold-plan-svg') as SVGElement | null;
    const planSvgStr = planSvgEl ? new XMLSerializer().serializeToString(planSvgEl) : '';

    // 全立面図 SVG を収集
    const elevSvgEls = document.querySelectorAll('[class^="elevation-svg-"]');
    const elevSvgs: string[] = [];
    const elevLabels: string[] = [];
    elevSvgEls.forEach(el => {
      elevSvgs.push(new XMLSerializer().serializeToString(el as SVGElement));
      // クラス名から面番号を取得
      const cls = (el as SVGElement).className?.baseVal ?? '';
      const match = cls.match(/elevation-svg-(\d+)/);
      elevLabels.push(match ? `外周面 ${parseInt(match[1]) + 1}` : '面');
    });
    ...
    const elevHtml = elevSvgs.map((svg, i) => `
      <div class="elev-block">
        <div class="elev-label">${elevLabels[i] ?? `面${i + 1}`}</div>
...
body { font-family:'Hiragino Sans','Yu Gothic UI','Meiryo',sans-serif; padding:10mm; font-size:10pt; }
h1 { font-size:14pt; color:#1B4F8A; border-bottom:2pt solid #1B4F8A; padding-bottom:5pt; margin-bottom:8pt; }
h2 { font-size:11pt; color:#1B4F8A; margin:10pt 0 6pt; border-left:3pt solid #E67E22; padding-left:6pt; }
.svg-wrap { width:100%; border:0.5pt solid #ddd; border-radius:3pt; overflow:hidden; margin-bottom:8pt; }
.svg-wrap svg { width:100%; height:auto; display:block; }
.ig { display:grid; grid-template-columns:repeat(3,1fr); gap:5pt; margin-bottom:10pt; }
.ic { background:#F2F4F6; border-radius:3pt; padding:4pt 7pt; }
.il { font-size:7.5pt; color:#7F8C8D; }
.iv { font-size:10pt; font-weight:700; color:#1B4F8A; }
.elev-block { margin-bottom:10pt; }
.elev-label { font-size:9pt; font-weight:700; color:#5D6D7E; margin-bottom:3pt; }
.footer { margin-top:8pt; font-size:7.5pt; color:#7F8C8D; border-top:0.5pt solid #ddd; padding-top:5pt; display:flex; justify-content:space-between; }
@media print {
  body { padding:6mm; }
...
<h1>仮設計画図 — ${projectName || '無題'}　${floors}F　H=${totalHeight.toFixed(1)}m　${verticalSegments}段</h1>

<h2>平面図</h2>
<div class="svg-wrap">${planSvgStr}</div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 12 }}>
        {(['plan', 'elevation'] as const).map(v => (
          <button
            key={v}
            onClick={() => setView(v)}
            style={{
              padding: '5px 14px', fontSize: 12, borderRadius: 4, border: '1px solid',
              cursor: 'pointer', fontWeight: view === v ? 700 : 400,
              background: view === v ? '#1B4F8A' : '#fff',
              color: view === v ? '#fff' : '#5D6D7E',
              borderColor: view === v ? '#1B4F8A' : '#CBD5E0',
            }}
          >
            {v === 'plan' ? '🗺️ 平面図' : '📐 立面図（各面）'}
          </button>
      ...
      {view === 'plan' && (
        <div style={{ background: '#FAFBFC', border: '1px solid #E5E8E8', borderRadius: 8, padding: 12, marginBottom: 12, overflowX: 'auto' }}>
          <svg className="scaffold-plan-svg" viewBox={`0 0 ${svgW} ${svgH}`} style={{ maxWidth: '100%', height: 'auto', display: 'block' }}>
            ...
            {wallTies.map((wt, i) => (
              <line key={i} x1={tx(wt.scaffold)} y1={ty(wt.scaffold)} x2={tx(wt.wall)} y2={ty(wt.wall)} stroke="#9B59B6" strokeWidth="0.8" opacity="0.5" />
            ))}
            ...
            {edgeLabels.map(({ x, y, label }, i) => (
              <text key={i} x={x} y={y} textAnchor="middle" fontSize="11" fill="#1B4F8A" fontWeight="600" style={{ paintOrder: 'stroke' as const }} stroke="white" strokeWidth="3">
                {label}
              </text>
            ))}
            ...
            <text x={svgW / 2} y="18" textAnchor="middle" fontSize="13" fill="#1B4F8A" fontWeight="700">仮設計画図（平面）</text>
      ...
      <p style={{ fontSize: 11, color: '#7F8C8D', marginTop: 8 }}>
        ※ PDFは印刷ダイアログで「PDFとして保存」を選択してください（Mac標準機能）。<br />
